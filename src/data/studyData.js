    ...
    materials: [
      {
        ...
        detail: {
          ...
          examFocus: [
            'Abstract Factory (추상 팩토리): 서로 연관된 객체 제품군을 생성하는 패턴',
            'Factory Method (팩토리 메서드): 하위 클래스가 생성할 구체 객체를 결정하는 패턴',
            'Singleton (싱글톤): 클래스의 인스턴스를 하나만 생성하고 공용 접근점을 제공하는 패턴',
            'Observer (옵저버): Subject의 상태 변화를 여러 의존 객체에 통지하는 패턴',
            'Strategy (전략): 교체 가능한 알고리즘을 별도 객체로 분리하는 패턴',
            'Adapter (어댑터): 호환되지 않는 인터페이스를 변환해 함께 사용하는 패턴',
          ],
        },
      },
    ],
